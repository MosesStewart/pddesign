...
def main():
    ...
    Y, W, D, Z, U = sim_bias(model_0, ndraws = 2000, seed = 1)
...
def plot_res(rres, pres, Y, D, fn):
    ...
    fig, ax = plt.subplots()
    ax.scatter(D, Y, s = 5, c = '#E7E7E7')
    ax.plot(x1, fn(x1), color = "#000000", label = 'True', linewidth = 2)
    ax.plot(x1, rres.predict(x1), color = '#96E7F1', label = 'RDD', linewidth = 2)
    ax.plot(x1, rres.predict(x1) + rres.left_ci - rres.est, color = "#96E7F1", linewidth = 1, alpha = 0.55)
    ax.plot(x1, rres.predict(x1) + rres.right_ci - rres.est, color = '#96E7F1', linewidth = 1, alpha = 0.55)
    ...
    ax.plot(x2, rres.predict(x2), color = '#96E7F1', linewidth = 2)
    ax.plot(x2, rres.predict(x2) + rres.left_ci - rres.est, color = '#96E7F1', linewidth = 1, alpha = 0.55)
    ax.plot(x2, rres.predict(x2) + rres.right_ci - rres.est, color = '#96E7F1', linewidth = 1, alpha = 0.55)
