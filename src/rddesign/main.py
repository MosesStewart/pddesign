...
def gaussian_kernel(u: torch.Tensor) -> torch.Tensor:
    ...
    return torch.exp(mynorm.log_prob(u)).to(torch.float32)
...
class rdd:
    ...
    def __build_matrices(self):
        ...
        vecB2β = {'+': self.Γ_2_inv['+'] @ self.R_2['+'].T @ self.L['+'] / self.n @ self.Y, 
                  '-': self.Γ_2_inv['-'] @ self.R_2['-'].T @ self.L['-'] / self.n @ self.Y}
        self.ε = {'+': (self.Y - self.R_2['+'] @ vecB2β['+']).unsqueeze(1),  # (n, q+1, 1)
                  '-': (self.Y - self.R_2['+'] @ vecB2β['-']).unsqueeze(1)}  # (n, q+1, 1)
        self.σ = {'+': self.ε['+'].abs(),  # (n, q+1, 1)
                  '-': self.ε['-'].abs(),}  # (n, q+1, 1)
        self.Σ = {'+': torch.diag((self.σ['+'][:, :, 0] ** 2).transpose(0, 1).contiguous().flatten()),
                  '-': torch.diag((self.σ['-'][:, :, 0] ** 2).transpose(0, 1).contiguous().flatten()),}
        ...
        self.P_bc = {'+': self.Γ_1_inv['+'] @ self.R_1['+'].T @ self.K['+'] - (self.h['+'] / self.b['+'])**2 * self.Γ_1_inv['+'] @ self.Λ_1['+'] @ self.e_2.T @ self.Γ_2_inv['+'] @ self.R_2['+'].T @ self.L['+'], 
                '-': self.Γ_1_inv['-'] @ self.R_1['-'].T @ self.K['-'] - (self.h['-'] / self.b['-'])**2 * self.Γ_1_inv['-'] @ self.Λ_1['-'] @ self.e_2.T @ self.Γ_2_inv['-'] @ self.R_2['-'].T @ self.L['-']}
        self.v_bc = torch.sqrt((self.h['+'] / self.n) * (self.e_0.T @ self.P_bc['+'] @ self.Σ['+'] @ self.P_bc['+'].T @ self.e_0) +\
                                (self.h['-'] / self.n) * (self.e_0.T @ self.P_bc['-'] @ self.Σ['-'] @ self.P_bc['-'].T @ self.e_0))
    ...
    def __get_q_1(self, sn: str, α=0.05):
        z = torch.tensor(norm.ppf(1 - α / 2), dtype=self.dtype, device=self.device)
        def proj_e0(L):  # L: (N,2,1) -> (N,)
            return torch.bmm(self.e_0.T.unsqueeze(0).expand(L.shape[0], -1, -1), L).squeeze(-1).squeeze(-1)
        
        # -------- term1..term4, term6, term8, term9, term12 (all i-only) --------
        mean1 = (((self.ε[sn][:, 0, 0] * proj_e0(self.ℓ_0_bc[sn]))**3) / self.h[sn]).mean()
        term1 = (self.v_bc**(-6) * (mean1**2) *
                (z**3 / 3 + 7*z/4 + self.v_bc**2 * z * (z**2 - 3)/4))
        mean2 = (((self.ε[sn][:, 0, 0] * proj_e0(self.ℓ_0_bc[sn])) * (self.ε[sn][:, 0, 0] * proj_e0(self.ℓ_1_bc_diag[sn]))) / self.h[sn]).mean()
        term2 = self.v_bc**(-2) * mean2 * (-(z**2 - 3) / 4)

        mean3 = ((((self.ε[sn][:, 0, 0] * proj_e0(self.ℓ_0_bc[sn]))**4) - ((self.σ[sn][:, 0, 0] * proj_e0(self.ℓ_0_bc[sn]))**4)) / self.h[sn]).mean()
        term3 = self.v_bc**(-4) * mean3 * (z * (z**2 - 3) / 8)

        mean4 = (((self.ε[sn][:, 0, 0] * proj_e0(self.ℓ_0_bc[sn]))**2) / self.h[sn] * (self.b[sn] * self.𝛿[sn].flatten()) * (self.R_2[sn] @ self.Γ_2_inv[sn] * self.R_2[sn]).sum(dim=1)).mean()
        term4 = self.v_bc**(-2) * mean4 * (z * (z**2 - 1) / 2)

        mean6 = ((((self.ε[sn][:, 0, 0] * proj_e0(self.ℓ_0_bc[sn]))**2) / (self.h[sn]**2)) * (self.b[sn] * self.𝛿[sn].flatten() * (self.R_2[sn] @ self.Γ_2_inv[sn] * self.R_2[sn]).sum(dim=1))**2).mean()
        term6 = self.v_bc**(-2) * mean6 * (z * (z**2 - 1)) / 4

        mean8 = (((self.ε[sn][:, 0, 0] * proj_e0(self.ℓ_0_bc[sn]))**4) / self.h[sn]).mean()
        term8 = self.v_bc**(-4) * mean8 * (-z * (z**2 - 3) / 24)

        submean = ((self.σ[sn][:, 0, 0] * proj_e0(self.ℓ_0_bc[sn]))**2).mean()  # scalar
        mean9 = ((((self.σ[sn][:, 0, 0] * proj_e0(self.ℓ_0_bc[sn]))**2 - submean) * ((self.ε[sn][:, 0, 0] * proj_e0(self.ℓ_0_bc[sn]))**2)) / self.h[sn]).mean()
        term9 = self.v_bc**(-4) * mean9 * (z * (z**2 - 1) / 4)

        mean12 = (((self.σ[sn][:, 0, 0] * proj_e0(self.ℓ_0_bc[sn]))**2 - submean) / self.h[sn]).mean()
        term12 = self.v_bc**(-4) * mean12 * (-z * (z**2 + 1) / 8)

        # -------- term5, term7 --------
        meana = (((((self.ε[sn][:, 0, 0] * proj_e0(self.ℓ_0_bc[sn]))**2) / self.h[sn]) * proj_e0(self.ℓ_0_bc[sn])).unsqueeze(1) * self.R_2[sn] @ self.Γ_2_inv[sn]).mean(dim=0, keepdim=True)  # (1,3)
        meanb = (((self.b[sn] / self.h[sn]) * self.𝛿[sn].flatten() * (self.ε[sn][:, 0, 0].pow(2) * proj_e0(self.ℓ_0_bc[sn]))).unsqueeze(1) * self.R_2[sn]).mean(dim=0).unsqueeze(1)    # (3,1)
        term5 = (self.v_bc**(-4) * (meana @ meanb).squeeze() * (z * (z**2 - 1)))

        mean7a = (((((self.ε[sn][:, 0, 0].pow(2) * proj_e0(self.ℓ_0_bc[sn]))) / self.h[sn]) * (self.b[sn] * self.𝛿[sn].flatten())).unsqueeze(1) * self.R_2[sn] @ self.Γ_2_inv[sn]).mean(dim=0, keepdim=True)    # (1,3)
        outer2 = self.R_2[sn].unsqueeze(2) * self.R_2[sn].unsqueeze(1)                 # (n,3,3) = R2_i^T R2_i
        mean7b = ((proj_e0(self.ℓ_0_bc[sn])**2) / self.h[sn]).view(self.n,1,1) * torch.matmul(outer2, self.Γ_2_inv[sn]) # (n,3,3)
        mean7b = mean7b.mean(dim=0)                                        # (3,3)
        mean7c = ((self.b[sn] / self.h[sn]) * self.𝛿[sn].flatten() * (self.ε[sn][:, 0, 0].pow(2) * proj_e0(self.ℓ_0_bc[sn]))).unsqueeze(1) * self.R_2[sn]           # (n,3)
        mean7c = mean7c.mean(dim=0).unsqueeze(1)                           # (3,1)
        term7 = (self.v_bc**(-4) * (mean7a @ mean7b @ mean7c).squeeze() * (z * (z**2 - 1) / 2))

        # -------- term10, term11 over sampled ordered pairs (I,J) --------
        M, I, J = self.M, self.I, self.J
        mean10 = ((((self.σ[sn][I, 0, 0] * torch.bmm(self.e_0.T.unsqueeze(0).expand(M, -1, -1), self.ℓ_1_bc[sn]).squeeze(-1).squeeze(-1)) *\
                    (self.σ[sn][:, 0, 0] * proj_e0(self.ℓ_0_bc[sn]))[I] * ((self.ε[sn][:, 0, 0] * proj_e0(self.ℓ_0_bc[sn]))[I]**2)) / (self.h[sn]**2)).mean())
        term10 = self.v_bc**(-4) * mean10 * (z * (z**2 - 3))

        mean11 = ((((self.ε[sn][:, 0, 0] * proj_e0(self.ℓ_0_bc[sn]))[I] * (self.ε[sn][I, 0, 0] * torch.bmm(self.e_0.T.unsqueeze(0).expand(M, -1, -1), self.ℓ_1_bc[sn]).squeeze(-1).squeeze(-1)) *\
                    ((self.σ[sn][:, 0, 0] * proj_e0(self.ℓ_0_bc[sn]))[I]**2 - submean)) / (self.h[sn]**2)).mean())
        term11 = self.v_bc**(-4) * mean11 * (z * (z**2 - 3))
    ...
    def __get_q_3(self, sn: str, α=0.05):
        z = torch.tensor(norm.ppf(1 - α / 2), dtype=self.dtype, device=self.device)
        mean3 = ((self.ε[sn][:, 0, 0] * torch.bmm(self.e_0.T.unsqueeze(0).expand(self.n, -1, -1), self.ℓ_0_bc[sn]).squeeze(-1).squeeze(-1))**3 / self.h[sn]).mean() 
        q_3 = - self.v_bc**(-4) * mean3 * z**3 / 3
        return q_3
    ...
    def __get_bandwidth(self, optim_mode = 'newton-cg', tol = 0.001):
        def obj(h):
            ...
            self.__build_edgeworth_terms()
            𝜇_3 = {'+': self.__get_𝜇_3('+'), '-': self.__get_𝜇_3('-')}
            𝜂_bc = {'+': torch.sqrt(self.n * self.h['+']) * self.h['+']**3 * 𝜇_3['+'] / factorial(3) * self.e_0.T @ self.Γ_1_inv['+'] @ (self.Λ_1_2['+'] - self.Λ_1['+'] @ self.e_2.T @ self.Γ_2_inv['+'] @ self.Λ_2_2['+']),
                    '-': torch.sqrt(self.n * self.h['-']) * self.h['-']**3 * 𝜇_3['-'] / factorial(3) * self.e_0.T @ self.Γ_1_inv['-'] @ (self.Λ_1_2['-'] - self.Λ_1['-'] @ self.e_2.T @ self.Γ_2_inv['-'] @ self.Λ_2_2['-'])}
            q_1 = {'+': self.__get_q_1('+'), '-': self.__get_q_1('-')}
            q_2 = self.__get_q_2()
            q_3 = {'+': self.__get_q_3('+'), '-': self.__get_q_3('-')}
            H = {'+': self.h['+'] * self.n**(1/5), '-': self.h['-'] * self.n**(1/5)}
    ...
    def fit(self):
        ...
        se = torch.sqrt((1 / self.n**2) * (self.e_0.T @ self.P_bc['+'] @ self.Σ['+'] @ self.P_bc['+'].T @ self.e_0) +\
                                (1 / self.n**2) * (self.e_0.T @ self.P_bc['-'] @ self.Σ['-'] @ self.P_bc['-'].T @ self.e_0))
