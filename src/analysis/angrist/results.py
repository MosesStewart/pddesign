...
def main():
    ...
    model = angrist_rdd(Y, D, cutoff = 40, kernel = 'triangle', bandwidth = [15, 15], weights = D, varbound = 2500, n_adjust = n_ratio)
    res_rdd = model.fit()
    print(res_rdd)
    
    model = angrist_pdd(Y, W, D, Z, cutoff = 40, kernel = 'triangle', bandwidth = [15, 15], weights = D, varbound = 2500, n_adjust = n_ratio)
    res_pdd = model.fit()
    print(res_pdd)
    ...
    model = angrist_rdd(W, D, cutoff = 40, kernel = 'triangle', bandwidth = [15, 15], weights = D, varbound = 2500, n_adjust = n_ratio)
    res_rdd = model.fit()
    print(res_rdd)
...
def _add_side_brackets(ax, res, color, x_offset_neg=0.15, x_offset_pos=-0.15,
...
def plot_res(rres, pres, Y, D):
    ...
    ax.plot(x2, pres.predict(x2) + 0.1, color='#FF6961', linewidth=2)
    # Right-side bracket: identical for RDD and PDD, draw once in black
    _add_side_brackets(ax, rres, color='#222222', x_offset_pos=-0.15, show_neg=False)
    # Left-side brackets: RDD further out, PDD closer in
    _add_side_brackets(ax, rres, color='#3aabb7', x_offset_neg=0.30, show_pos=False)
    _add_side_brackets(ax, pres, color='#d94f48', x_offset_neg=0.15, show_pos=False)
    ax.vlines(x=40, ymin=50, ymax=75, color='#000000', alpha=0.5, linestyle=(0, (8, 8)))
...
def plot_rdd(res, Y, D):
    ...
    _add_side_brackets(ax, res, color='#d94f48', x_offset_neg=0.15, x_offset_pos=-0.15)
    ax.vlines(x=40, ymin=50, ymax=75, color='#000000', alpha=0.5, linestyle=(0, (8, 8)))
    #ax.legend(loc='upper left')
    ax.set_xlabel('D')
    ax.set_xlim(35, 46)
